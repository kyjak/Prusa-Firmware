...
{
  if (check_endstops || SCANNER_MODE == 1)
  {
#ifndef COREXY
    ...
    { // +direction
      ...
      // Stall guard homing turned on
      if (SCANNER_MODE == 1)
        x_max_endstop = (READ(X_MIN_PIN) != X_MIN_ENDSTOP_INVERTING);
      else
...
#endif
    {
      ...
      // Stall guard homing turned on
      if (SCANNER_MODE == 1)
        y_min_endstop = (READ(X_MIN_PIN) != X_MIN_ENDSTOP_INVERTING);
      else
    ...
    { // +direction
      ...
      // Stall guard homing turned on
      if (SCANNER_MODE == 1)
        y_max_endstop = (READ(X_MIN_PIN) != X_MIN_ENDSTOP_INVERTING);
      else
    ...
    if ((out_bits & (1 << Z_AXIS)) != 0) // -direction
    {
      ...
      if (!check_z_endstop)
      {
        ...
        // Stall guard homing turned on
        if (SCANNER_MODE == 1)
          z_min_endstop = (READ(X_MIN_PIN) != X_MIN_ENDSTOP_INVERTING);
        else
        {
...
#endif
                ? INVERT_E0_DIR
                : !INVERT_E0_DIR; //If we have SNMM, reverse every second extruder.
