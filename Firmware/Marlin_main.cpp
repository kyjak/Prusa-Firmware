...
*/
    ...
    */
      ...
      case 920:
        if (code_seen('X'))
          X_MAX_ENDSTOP_INVERTING = code_value_uint8();
        if (code_seen('Y'))
          Y_MAX_ENDSTOP_INVERTING = code_value_uint8();
        if (code_seen('Z'))
          Z_MAX_ENDSTOP_INVERTING = code_value_uint8();
        SERIAL_PROTOCOLLNRPGM(MSG_OK);
        break;
      ...
      case 921:
        if (code_seen('X'))
          X_MIN_ENDSTOP_INVERTING = code_value_uint8();
        if (code_seen('Y'))
          Y_MIN_ENDSTOP_INVERTING = code_value_uint8();
        if (code_seen('Z'))
          Z_MIN_ENDSTOP_INVERTING = code_value_uint8();
        SERIAL_PROTOCOLLNRPGM(MSG_OK);
        break;
      case 922:
        if (code_seen('Y'))
  ...
  */
    case 80:
    {
      ...
      if (code_seen('E'))
        dimension_x = code_value();
      if (code_seen('F'))
        dimension_y = code_value();
      if (code_seen('G'))
      {
        points_x = code_value();
      }
